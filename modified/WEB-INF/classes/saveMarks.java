...
public class saveMarks extends HttpServlet {
  ...
  String ofinal=request.getParameter("ofinal");
    ...
    out.println("<html>");
    out.println("<head><title>ADD MARKS</title><link rel='stylesheet' href='css/bootstrap.min.css'/></head>");
    ...
    }else
{
  ...
  try
  {
    ...
    String teachers=(String)session.getAttribute("session_teachers");  	
     ...
     boolean check2=true;
     boolean check3=true;
    String query1 ="Select * from add_course where courses='"+courses+"' AND teachers='"+teachers+"'";
    ResultSet rs1 = st.executeQuery( query1 );
  
   if(!(rs1.next()))
   {  
        out.println("<h1 class='text-center mt-3'>Course: "+courses+" has no enrolled students </h1><br/>");
    check1=false;
      }
